...
import { 
  ...
  InputLabel, Box, Typography, Divider, Grid 
} from '@mui/material';
import { PLANS, STATUSES } from '../utils/subscriptionUtils';
import { updateSubscriptionService } from '../../../services/empresas.services';

export default function BillingManager({ open, onClose, company, token, onUpdateSuccess }) {
  ...
  useEffect(() => {
    if (company) {
      setFormData({
        plan: company.subscription?.plan || PLANS.FREE,
        status: company.subscription?.status || STATUSES.ACTIVE,
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      await updateSubscriptionService(token, company.id, formData);
      onUpdateSuccess();
      onClose();
    } catch (error) {
