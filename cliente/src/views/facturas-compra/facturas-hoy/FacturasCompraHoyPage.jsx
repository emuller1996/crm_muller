...
FacturasCompraHoyPage.propTypes = {
  draw: PropTypes.number,
  onPayment: PropTypes.func,
  onViewFactura: PropTypes.func,
}

export default function FacturasCompraHoyPage({ onViewFactura, onPayment, draw }) {
  ...
  const getFacturasPerDay = async (dateS) => {
    try {
      setLoading(true)
      const result = await getAllFacturaCompraPerDay(dateS)
      ...
      <div className="rounded overflow-hidden border border-ligth shadow-sm mt-3">
        <DataTable
          ...
          columns={[
            ...
            { name: 'N° Factura', selector: (row) => `FC-${row?.numero_factura}` ?? '' },
